...
async fn test_executor() -> Result<(), Box<dyn Error>> {
    let requests = HashMap::<String, reqwest::RequestBuilder>::from([
        (
            String::from("test_http_get"),
            reqwest::Client::new().get("http://localhost:7878/test_http_methods_endpoint"),
        ),
        (
            String::from("test_http_post"),
            reqwest::Client::new().post("http://localhost:7878/test_http_methods_endpoint"),
        ),
        (
            String::from("test_http_unknown"),
            reqwest::Client::new().get("http://localhost:2323"),
        ),
    ]);
